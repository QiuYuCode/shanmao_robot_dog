...
灵巧手协议使用的是　modbustcp 协议，协议内容参考如下

> python 可用　pymodbus 模块开发，虚拟环境已配置改模块

详细内容请参考资料文档

示例指令：
```bash
EB 90 01 0F 12 10 04 E8 03 E8 03 E8 03 E8 03 E8 03 E8 03 B8
```

读取灵巧手寄存器操作
```bash
0xEB 0x90 hands_id 0x0F 0x12 address_l addless_h register_length checksum
```
- 其中0xEB和0x90是包头
- hands_id 默认为1 灵巧手的id
- 0x0F 指的是数据长度 具体指的是：12 10 04 E8 03 E8 03 E8 03 E8 03 E8 03 E8 03
- 0x11是读取寄存器
- 0x12是写入取寄存器
- 10 04是指的寄存器起始地址低八位和高八位  这个值对应的是大拇指的地址  这里只需要填初始值即可
- address_L 是指的寄存器起始地址低八位 如E8
- address_H 是指的寄存器起始地址高八位 如03
- checksum 校验和指的是除包头外所有数据的和值的低八位

其中第一个数据位（也就是第一个e8 03）为小拇指然后依次就是四个手指
倒数第二位为大拇指内旋位（往小拇指方向）  最后一位是大拇指往食指位


写入握拳指令：
将 ID 为 1 的灵巧手各自由度角度均设置为 1000：
指令帧：
...
写入手掌指令：
将 ID 为 1 的灵巧手四个手指为1740 大拇指弯曲1550 大拇指往内移动1800：

```bash
EB 90 01 0F 12 10 04 cc 06 cc 06 cc 06 cc 06 08 07 0e 06 a1
```

写入单食指伸出（可用于按键）

```bash
EB 90 01 0F 12 10 04 84 03 84 03 84 03 cc 06 1a 04 49 03 07
```
